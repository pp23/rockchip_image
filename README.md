...
* 29.09.25: Using ramdisk with minimal busybox. No console could get open so far, but the /init gets loaded:

```
ext4load mmc 1 ${kernel_addr_r} boot/vmlinuz-6.15.7
17233221 bytes read in 1437 ms (11.4 MiB/s)
=> ext4load mmc 1 ${fdt_addr_r} boot/rk3588-orangepi-5-plus.dtb
94948 bytes read in 11 ms (8.2 MiB/s)
=> ext4load mmc 1 ${ramdisk_addr_r} boot/uInitrd.busybox
1835512 bytes read in 156 ms (11.2 MiB/s)
=> setenv bootargs "rdinit=/init splash=verbose earlycon console=ttyFIQ0,1500000n8 consoleblank=0 loglevel=15 raid=noautodetect clk_ignore_unused"
=> booti ${kernel_addr_r} ${ramdisk_addr_r} ${fdt_addr_r}
```

* Console printing logs with the Armbian sdcard, selfcompiled kernel, dtb in /boot/:
```
ext4load mmc 1 ${kernel_addr_r} boot/vmlinuz-6.15.7
ext4load mmc 1 ${fdt_addr_r} boot/rk3588-orangepi-5-plus.dtb
setenv bootargs "root=PARTUUID=882c82c6-01 rootwait rootfstype=ext4 init=/bin/sh splash=verbose earlycon console=ttyS0,1500000 consoleblank=0 loglevel=15 raid=noautodetect clk_ignore_unused"
booti ${kernel_addr_r} - ${fdt_addr_r}
```

* Problems detected: clk_ignore_unused required to avoid hangup at `[    1.027387] clk: Disabling unused clocks` (see https://www.kernel.org/doc/html/v6.1/driver-api/clk.html#disabling-clock-gating-of-unused-clocks)
* The DTB seems to have a wrong sdcontroller config. Without `rootwait`, this error gets printed:
  ```
...
* or try to fix the DTB somehow

#### Kernel Debug Log with tp_printk traces of serial/tty:

bootargs: `rdinit=/init splash=verbose earlycon console=ttyS2,1500000n8 loglevel=8 raid=noautodetect clk_ignore_unused pd_ignore_unused regulator_ignore_unused tp_printk trace_event=tty:* trace_event=serial:* initcall_debug`
